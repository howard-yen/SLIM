...
```bash
# this is necessary for all agent settings
...
We use Azure OpenAI by default, but you can easily switch to the other providers by modifying the `model` argument in the `simple_evals.py` file.
We use [LiteLLM](https://github.com/BerriAI/litellm) to query model endpoints, so you may also use other providers supported by LiteLLM (see [here](https://docs.litellm.ai/docs/providers)).
